...
export const filterProfiles = (
  ...
  if (location) {
    ...
    filtered = filtered.filter(profile =>
      profile.address.city.toLowerCase().includes(lowerLocation) ||
      profile.address.state.toLowerCase().includes(lowerLocation) ||
      profile.address.country.toLowerCase().includes(lowerLocation)
    );
  }

  if (interests && interests.length > 0) {
    filtered = filtered.filter(profile =>
      profile.interests && interests.every(interest =>
...
export const sortProfiles = (
  profiles: Profile[],
  sortBy: ProfileFilterOptions['sortBy'] = 'name',
  sortOrder: ProfileFilterOptions['sortOrder'] = 'asc'
): Profile[] => {
  ...
  sorted.sort((a, b) => {
    ...
    switch (sortBy) {
      case 'createdAt':
      case 'updatedAt':
        valA = a[sortBy].getTime();
        valB = b[sortBy].getTime();
        break;
      case 'name':
        ...
        break;
      default:
        valA = a[sortBy as keyof Profile];
        valB = b[sortBy as keyof Profile];
